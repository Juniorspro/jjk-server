  ...
  ws.on('message', (raw) => {
    ...
    try {
      switch (msg.type){
        ...
        case 'propUpdate': {
          // ⭐ v13: server-authoritative para props NO agarrados.
          // Mientras está GRABBED, el grabber es autoridad y manda propUpdate.
          // El server reenvía a los demás (no espera al snapshot tick que skipea grabbed).
          if (!player.room) break;
          const room = rooms.get(player.room);
          ...
          // ⭐ FIX: reenviar a todos LOS DEMÁS (no al sender). Si no, el espectador
          // ve el cubo congelado durante el grab porque el snapshot tick skipea grabbed.
          if (validUpdates.length > 0){
            broadcastToRoom(player.room, { type: 'propUpdate', props: validUpdates }, playerId);
          }
          break;
        }
        case 'propGrab': {
