...
// - Room system: 3 pinned global rooms + user-created public/private rooms
// - Password protection for private rooms
// - Player limits: 30 for pinned, 20 for user-created
// ============================================================
...
const rooms = new Map();     // roomId -> { name, isPinned, isPublic, passwordHash, players, dummies, nextDummyId, maxPlayers }
...
// Generate a short room code for user-created rooms
...
function getPublicRoomList(){
  ...
  // Then user-created public rooms
  for (const r of rooms.values()){
...
function leaveCurrentRoom(player){
  ...
  // Clean up this player's radios + broadcast stops
  if (oldRoom.radios){
    for (const [rid, r] of [...oldRoom.radios]){
      if (r.ownerId === player.id){
        oldRoom.radios.delete(rid);
        broadcastToRoom(player.room, { type: 'radio_stop', radioId: rid });
      }
    }
  }
  broadcastToRoom(player.room, { type: 'leave', id: player.id });
  // Delete room if empty AND not pinned
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/'){
    ...
    // HTTP fallback for browsing rooms (also available via WS message)
    res.writeHead(200, {'Content-Type':'application/json', 'Access-Control-Allow-Origin':'*'});
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type){
      ...
      // List all public rooms (for the lobby UI)
      case 'listRooms': {
        sendTo(playerId, {
          type: 'roomList',
      ...
      // Create a new room. Returns room info + auto-joins.
      // { type:'createRoom', name, isPublic, password (optional), playerName, color }
      case 'createRoom': {
      ...
      // { type:'joinRoom', roomId, password (optional), playerName, color }
      case 'joinRoom': {
        ...
        if (room.radios){
          for (const r of room.radios.values()){
            existingRadios.push({
              radioId: r.radioId, ownerId: r.ownerId,
      ...
      // Legacy 'join' kept for backward compat — joins the named room (creating if missing as public room)
      case 'join': {
        const roomId = (msg.room || 'global').slice(0, 32);
        let room = rooms.get(roomId);
        if (!room){
          // Create as public, no password
          room = {
        ...
        const existingRadios2 = [];
        if (room.radios){
          for (const r of room.radios.values()){
            existingRadios2.push({
              radioId: r.radioId, ownerId: r.ownerId,
              videoId: r.videoId, title: r.title,
              isPersonal: r.isPersonal,
              x: r.x, y: r.y, z: r.z,
        ...
        sendTo(playerId, { type: 'roomState', roomId: room.id, roomName: room.name, players: existingPlayers, dummies: existingDummies, radios: existingRadios2 });
        broadcastToRoom(room.id, {
      ...
      case 'radio_play': {
        ...
        const room = rooms.get(player.room);
        if (!room) break;
        if (!room.radios) room.radios = new Map();
        // Limit: max 5 active radios per room
        if (room.radios.size >= 5){
          // If owner already has one, remove it first
          for (const [rid, r] of room.radios){
            if (r.ownerId === playerId){
              room.radios.delete(rid);
              broadcastToRoom(player.room, { type: 'radio_stop', radioId: rid });
              break;
            }
          }
          if (room.radios.size >= 5){
            sendTo(playerId, { type: 'chat', id: 'system', name: 'Server',
              text: '⚠️ Máximo 5 radios por sala. Esperá a que otra termine.' });
            break;
          }
        }
        const radioId = String(msg.radioId || '').slice(0, 64);
        const videoId = String(msg.videoId || '').slice(0, 16);
        const title = String(msg.title || 'Música').slice(0, 200);
        if (!radioId || !videoId) break;
        const radio = {
          radioId,
          ownerId: playerId,
          videoId,
          ...
          y: Number(msg.y) || 0,
          z: Number(msg.z) || 0,
          skin: String(msg.skin || 'classic').slice(0, 20),
          startedAt: Date.now(),
        };
        room.radios.set(radioId, radio);
        // Broadcast to everyone INCLUDING owner so client can confirm
        broadcastToRoom(player.room, {
          type: 'radio_play',
          radioId, fromId: playerId,
          videoId, title,
          isPersonal: radio.isPersonal,
          x: radio.x, y: radio.y, z: radio.z,
          skin: radio.skin,
        });
        console.log(`[radio] ${playerId} play "${title}" (${videoId})`);
        break;
      }
      ...
      case 'radio_stop': {
        if (!player.room) break;
        const room = rooms.get(player.room);
        if (!room || !room.radios) break;
        const radioId = String(msg.radioId || '').slice(0, 64);
        const r = room.radios.get(radioId);
        if (!r) break;
        // Only owner can stop their own radio
        if (r.ownerId !== playerId) break;
        room.radios.delete(radioId);
        broadcastToRoom(player.room, { type: 'radio_stop', radioId });
        console.log(`[radio] ${playerId} stop ${radioId}`);
        break;
      }

      case 'radio_sync': {
        // Owner broadcasts current playback time so others can sync
        if (!player.room) break;
        const room = rooms.get(player.room);
        if (!room || !room.radios) break;
        const radioId = String(msg.radioId || '').slice(0, 64);
        const r = room.radios.get(radioId);
        if (!r || r.ownerId !== playerId) break;
        broadcastToRoom(player.room, {
          type: 'radio_sync', radioId,
