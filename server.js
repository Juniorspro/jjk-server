// ============================================================
// JJK 24FRAMES — Sandbox Multiplayer Server (v6)
// ============================================================
    ...
    switch (msg.type){
      ...
      case 'propSpawn': {
        ...
        room.props.set(msg.mpId, {
          ...
          qx: 0, qy: 0, qz: 0, qw: 1,
          scale: +msg.scale || 1,           // ⭐ nuevo
          color: msg.color || null,         // ⭐ nuevo (hex string)
          frozen: false,                    // ⭐ nuevo
          grabbedBy: null,                  // ⭐ nuevo (playerId que lo tiene agarrado)
          ownerId: playerId,
        });
        broadcastToRoom(player.room, {
          type: 'propSpawn',
      ...
      case 'propUpdate': {
        ...
        for (const p of msg.props){
          ...
          // Permitir update si: es dueño O lo tiene agarrado
      ...
      // ⭐ Nuevo: agarrar prop con grav gun
      case 'propGrab': {
        if (!player.room) break;
        const room = rooms.get(player.room);
        ...
        if (!stored) break;
        // No se puede agarrar si otro jugador ya lo tiene
      ...
      // ⭐ Nuevo: congelar/descongelar prop
      case 'propFreeze': {
        if (!player.room) break;
        const room = rooms.get(player.room);
        if (!room || !msg.mpId) break;
        ...
        if (stored.frozen) stored.grabbedBy = null; // freeze libera el grab
        broadcastToRoom(player.room, {
          type: 'propFreeze',
          mpId: msg.mpId,
  ...
  ws.on('close', () => {
    ...
    if (player.room){
      ...
      if (room){
        room.players.delete(playerId);
        broadcastToRoom(player.room, { type: 'leave', id: playerId });
        // ⭐ MP SYNC: liberar props que el jugador tenía agarrados (no borrarlos)
        // Los props persisten en la sala incluso si el dueño se va
        if (room.props){
          for (const [mpId, p] of room.props){
            if (p.grabbedBy === playerId){
              p.grabbedBy = null;
              broadcastToRoom(player.room, { type: 'propRelease', mpId, x: p.x, y: p.y, z: p.z, vx:0, vy:0, vz:0 });
            }
          }
        }
